...
def build_tensor_data(df):
    ...
    G_LIQ_tdt = np.full((n_co, n_cr, n_fe, n_T), np.nan)
    G_FCC_tdt = np.full((n_co, n_cr, n_fe, n_T), np.nan)
    ...
    return {
        ...
        'co_vals': co_vals, 'cr_vals': cr_vals,
        'fe_vals': fe_vals, 'T_vals': T_vals,
...
def cpd_als_4d(tensor, rank, max_iter=100, tol=1e-6):
    """4-way CP decomposition via ALS with NaN handling"""
    I, J, K, L = tensor.shape
    mask = ~np.isnan(tensor)
    X = np.where(mask, tensor, 0)

    # Initialize with SVD of mode-1 unfolding for better convergence
    U, s, Vh = linalg.svd(unfold_tensor(X, 0), full_matrices=False)
    ...
    for iteration in range(max_iter):
        ...
        for l in range(L):
            valid = mask_flat[l, :]
            if np.sum(valid) > rank:
                D[l, :] = linalg.lstsq(ABC[valid, :], X_flat[l, valid])[0]
        ...
        error = np.sqrt(np.sum(mask * (X - recon)**2) / np.sum(mask))

        if abs(prev_error - error) < tol:
            break
        prev_error = error

    # Store norms in lambda
    lam = np.ones(rank)
    for r in range(rank):
...
with tab_tensor:
    ...
    **Canonical Polyadic Decomposition (CPD)** to quantify rank, compression, and separability.
    ...
    | Property | Value |
    |----------|-------|
    | **TDT Order** | 4 (Co x Cr x Fe x T) |
    | **Full hypercube** | {full_size:,} entries |
    | **Valid simplex entries (G_LIQ)** | {valid_liq:,} ({100*valid_liq/full_size:.1f}%) |
    | **Valid simplex entries (G_FCC)** | {valid_fcc:,} ({100*valid_fcc/full_size:.1f}%) |
    ...
    reduction = (1 - cpd_coeffs / valid_liq) * 100

    st.markdown(f"""
    | Metric | Value |
    |--------|-------|
    | **CP Rank (R)** | {R_test} |
    | **CPD coefficients** | R x (I+J+K+L) = {R_test} x ({n_co}+{n_cr}+{n_fe}+{n_T}) = **{cpd_coeffs:,}** |
    ...
    if st.button("⚙️ Run CPD-ALS (may take 1-2 min for large tensors)", use_container_width=True):
        with st.spinner(f"Running CP-ALS with R={R_test}..."):
            ...
            # Reconstruction quality visualization - slice at fixed T
    ...
    with st.expander("📖 Tensor Decomposition Theory", expanded=False):
        ...
        Each entry of the tensor is approximated by:

        T(i,j,k,l) = sum over r=1 to R of: lambda_r * A(i,r) * B(j,r) * C(k,r) * D(l,r)

        where:
        - R = rank (number of separable components)
        ...
        **Key advantages:**
        ...
        2. **Incomplete tensor handling**: Only simplex-valid entries are used in fitting
        3. **Polynomial constraints**: Factor vectors can be constrained to polynomials
