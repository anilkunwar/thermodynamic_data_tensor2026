...
st.set_page_config(page_title="CoCrFeNi Gibbs Free Energy Explorer", layout="wide")
...
with st.sidebar:
    ...
    if render_mode == "Spherical Harmonic Surface (SH)":
...
if render_mode == "Spherical Harmonic Surface (SH)":
    ...
    if not np.any(valid_mask):
        ...
        if coeffs is not None:
            ...
            # Use a colormap from the list; default to Viridis
            ...
            # Optional: show the original sampled points as small markers (for reference)
            # (commented to keep clean, can be enabled)
            # fig.add_trace(go.Scatter3d(
            #     x=X[valid_mask].ravel(), y=Y[valid_mask].ravel(), z=Z[valid_mask].ravel(),
            #     mode="markers", marker=dict(size=1, color="black"), showlegend=False
            # ))
        else:
