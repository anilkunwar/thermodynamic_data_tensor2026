...
SIGN CONVENTION: Aligned with standard thermodynamic theory (Porter & Easterling, Christian)
  - ΔG = G_FCC - G_LIQ  [J/mol]
  - P_chem = -ΔG / V_m = (G_LIQ - G_FCC) / V_m  [Pa]
  - P_net = P_chem - P_capillary  [Pa]
  - Positive P_net drives LIQUID → FCC (growth)
  - Critical radius: r* = 2γ / P_chem [m]
"""
import os
import sys
import glob
...
**Critical Nucleus:** $r^* = 2\gamma/P_{chem}$
""")
...
def compute_capillary_pressure(gamma, curvature_r):
    """Capillary pressure: P_capillary = 2γ/r [Pa]. Always positive, opposes growth."""
    return (2.0 * gamma) / curvature_r

def compute_net_pressure(P_chem, P_capillary):
    """Net pressure: P_net = P_chem - P_cap [Pa]. Positive drives LIQ→FCC."""
...
def compute_critical_radius(gamma, P_chem):
    """Critical nucleation radius: r* = 2γ/P_chem [m]. Only valid when P_chem > 0."""
    if P_chem <= 0:
        return np.nan
    return 2.0 * gamma / P_chem
...
def display_latex_theory():
    ...
    with st.expander("📘 View Theory Explanation", expanded=True):
        ...
        st.markdown("""
        The chemical driving pressure follows the standard thermodynamic convention
        (Porter & Easterling, Christian):
        """)
        st.markdown(r"""
        $$
        P_{\text{chem}} = -\frac{\Delta G}{V_m} = \frac{G_{\text{LIQ}} - G_{\text{FCC}}}{V_m}
        $$
        """)
        st.markdown(r"""
        Unit conversion:
        ...
        """)
        st.success("""
        Positive $P_{\text{chem}}$ drives LIQUID → FCC transformation.
        Negative $P_{\text{chem}}$ drives FCC → LIQUID (remelting).
        """)

        st.markdown("### 5. Critical Nucleation Radius")
        st.markdown("""
        For homogeneous nucleation, the critical radius is:
        """)
        st.markdown(r"""
        $$
        r^* = \frac{2\gamma}{P_{\text{chem}}}
        $$
        """)
        st.info("""
        - Only defined when P_chem > 0 (FCC thermodynamically favored)
        - Nuclei with r < r* will dissolve; r > r* will grow
        - Represents the energy barrier for phase transformation
        """)

        st.markdown("### 6. Interface Force from Direct Area")
        st.markdown("""
        If the interface area is supplied directly, the force is:
        """)
        st.markdown(r"""
        $$
        F = P_{\text{chem}} \cdot A
        $$
        """)
        st.markdown(r"""
        ...
        st.markdown("### 7. Grain-Size-Derived Interface Area")
        st.markdown("""
        ...
        st.markdown("### 8. Net Driving Force on All Grain Boundaries")
        st.markdown("""
        ...
        F_{\mathrm{total}}
        =
        P_{\text{chem}} \cdot A_{\mathrm{total}}
        =
        P_{\text{chem}} \frac{kV}{d}
        $$
        """)
        st.warning("""
        ...
        st.markdown("### 9. Capillary-Corrected Differential Force")
        st.markdown(r"""
        ...
        $$
        P_{\mathrm{net}} = P_{\text{chem}} - P_{\mathrm{capillary}}
        $$
        ...
        st.markdown("### 10. Temperature Normalization")
        st.markdown("""
        ...
        st.markdown("### 11. Interpolation Strategy")
        st.markdown("""
        ...
        st.markdown("### 12. Key Assumptions")
        st.markdown("""
        ...
        - Capillary correction uses isotropic interfacial energy γ.
        """)
        ...
        5. Christian, J.W. *The Theory of Transformations in Metals and Alloys*.
        6. SciPy documentation for `RegularGridInterpolator` and `LinearNDInterpolator`.
        """)
...
# ⚠️ IMPROVEMENT #2: Warning when capillary enabled but grain-size mode not active
if use_capillary and area_mode == "Direct Input (A)":
    st.sidebar.warning("⚠️ Capillary correction requires Grain Size Derived mode. Switch mode or disable capillary.")

gamma = GAMMA_LIQUID_FCC
if use_capillary and area_mode == "Grain Size Derived (Sv x V)":
...
if g_liq is None or g_fcc is None:
    ...
    # ------------------------------------------------------------
    # CORRECTED: Standard thermodynamic sign convention
    # P_chem = -ΔG / V_m  [Pa]
    # Positive P_chem when FCC is favored (ΔG < 0)
    # ------------------------------------------------------------
    P_chem = -delta_G / V_m  # Chemical driving pressure [Pa]
    P_chem_MPa = P_chem / 1e6  # MPa

    # ⚠️ IMPROVEMENT #3: Critical radius calculation
    r_critical = compute_critical_radius(gamma if use_capillary else GAMMA_LIQUID_FCC, P_chem)
    r_critical_nm = r_critical * 1e9 if not np.isnan(r_critical) else None

    # ------------------------------------------------------------
    # Corrected pressure and force calculation
    # ------------------------------------------------------------
    if use_capillary and grain_size_m is not None and area_mode == "Grain Size Derived (Sv x V)":
    ...
    # Display pressure metrics with critical radius
    if use_capillary and grain_size_m is not None and area_mode == "Grain Size Derived (Sv x V)":
        col_p1, col_p2, col_p3, col_p4, col_p5 = st.columns(5)
        col_p1.metric(
            "Chemical Drive P_chem",
            f"{P_chem_MPa:.3f} MPa",
            help="P_chem = -ΔG/Vₘ = (G_LIQ - G_FCC)/Vₘ. Positive when FCC is thermodynamically favored."
        )
        col_p2.metric(
            "Capillary Pressure",
        ...
        # ⚠️ IMPROVEMENT #3: Display critical radius
        if r_critical_nm is not None:
            col_p4.metric(
                "Critical Radius r*",
                f"{r_critical_nm:.2f} nm",
                help=f"r* = 2γ/P_chem; nuclei > r* will grow"
            )
        else:
            col_p4.metric("Critical Radius r*", "N/A", help="P_chem ≤ 0: no nucleation barrier")
        # CORRECTED: Positive P_net drives LIQUID → FCC (growth)
        direction = "→ LIQUID → FCC (Growth)" if P_net > 0 else "→ FCC → LIQUID (Remelting)"
        col_p5.metric("Interface motion", direction)
        ...
        **Capillary-Corrected Driving Pressure:**
        ...
        - Critical nucleus radius: **{r_critical_nm:.2f} nm** (if P_chem > 0)
        """)
    else:
        ...
        # ⚠️ IMPROVEMENT #3: Display critical radius even in direct mode
        if r_critical_nm is not None:
            col_p3.metric(
                "Critical Radius r*",
                f"{r_critical_nm:.2f} nm",
            ...
            col_p3.metric("Critical Radius r*", "N/A", help="P_chem ≤ 0: no nucleation barrier")
        # CORRECTED: Positive P_chem (or P_net) drives LIQUID → FCC
        direction = "→ LIQUID → FCC (Growth)" if P_net > 0 else "→ FCC → LIQUID (Remelting)"
...
with tab2:
    ...
    for T_val in T_scan:
        ...
        if gl is not None and gf is not None:
            ...
            # ⚠️ IMPROVEMENT #1: Use consistent vm_model across tabs
            vm_local = composition_dependent_vm(x_co, x_cr, x_fe, x_ni) if vm_model == "Composition‑dependent" else V_m
            # CORRECTED: P_chem = -ΔG / V_m
            P_chem_mpa_list.append(-dG / vm_local / 1e6)
    ...
    if not valid_T:
        ...
        fig2.update_layout(
            ...
            yaxis2=dict(title="P_chem (MPa)", titlefont=dict(color="#d62728"),
        ...
        - **Red dotted curve (P_chem)**: Chemical driving pressure in MPa. Positive values drive LIQUID → FCC
        - **Gray dashed line**: Phase boundary (ΔG = 0, equivalently P_chem = 0)
        - Crossing points indicate phase transition temperatures
        """)

with tab3:
    st.markdown("### Chemical Driving Pressure vs Composition (P_chem in MPa)")
    scan_var2 = st.radio("Scan along", ["x_Co", "x_Cr", "x_Fe"], horizontal=True, key="scan_pchem")
    fixed_val2 = st.slider("Fixed other components", 0.0, 0.4, 0.2, 0.01, key="fixed_pchem")
    ...
    if max_val2 < 0.01:
        ...
        for xv in x_vals2:
            ...
            if gl is not None and gf is not None:
                ...
                # CORRECTED: P_chem = -(gf - gl) / V_m
                P_chem_mpa = -(gf - gl) / vm_local / 1e6
                P_chem_vals.append(P_chem_mpa)
                valid_x2.append(xv)
            else:
                P_chem_vals.append(np.nan)
        fig3 = go.Figure()
        fig3.add_trace(go.Scatter(
            x=valid_x2, y=P_chem_vals,
        ...
        if scan_var2 == "x_Co":
            ...
            current_x2 = x_fe
        current_P_chem = -delta_G / V_m / 1e6 if g_liq is not None else None
        if current_P_chem is not None:
            fig3.add_trace(go.Scatter(
                x=[current_x2], y=[current_P_chem],
    ...
    with st.spinner("Building multi-ring chart..."):
        ...
        for iT, T_sun in enumerate(temperatures_sorted):
            ...
            for j, row in sample_df.iterrows():
                ...
                # ⚠️ IMPROVEMENT #1: Use consistent vm_model across tabs in multi-ring
                if vm_model == "Constant":
                    V_m_s = V_m  # Use the user-selected constant V_m from sidebar
                else:
                    V_m_s = composition_dependent_vm(x_co_s, x_cr_s, x_fe_s, x_ni_s)

                # CORRECTED: P_chem = -ΔG / V_m  [Pa]
                ...
                for el in ["Co", "Cr", "Fe", "Ni"]:
                    ...
                    element_data[el]["custom"].append([
                        ...
                        x_ni_s,
                        delta_G_s,
                        P_chem_mpa_s,    # index 6
                        net_force_s,
                        el,
                        comp_vals[el],
                        P_net_mpa_s,     # index 10
                        force_mode_s,
                    ])
...
with tab6:
    ...
    if g_liq is None or g_fcc is None:
        ...
        # Use absolute P_chem for normalized magnitude display
        P_chem_MPa_abs = abs(P_chem_MPa) if 'P_chem_MPa' in locals() else 0.0
        P_chem_norm = min(1.0, P_chem_MPa_abs / 100)
        categories = ['x_Co', 'x_Cr', 'x_Fe', 'x_Ni', 'T (norm)', '|P_chem| (norm)']
        values = [x_co, x_cr, x_fe, x_ni, T_norm, P_chem_norm]
        phase_pref, phase_color, phase_emoji = get_phase_preference(delta_G)
        fig_radar = go.Figure(data=go.Scatterpolar(
            ...
            hovertemplate=
                ...
                '|P_chem|: %{r[5]:.3f} (norm) ≈ {P_chem_MPa_abs:.2f} MPa<br>' +
...
4. Christian, J.W. *The Theory of Transformations in Metals and Alloys*, Elsevier (2002)
5. SciPy: `RegularGridInterpolator`, `LinearNDInterpolator` documentation
6. Plotly: Interactive visualization library (https://plotly.com)
7. Underwood, E.E. *Quantitative Stereology*, Addison-Wesley (grain shape factors & Sv)
...
**Grain Size Method**: $S_v = k/d$; $A_{{total}} = S_v \times V$; $F_{{total}} = P_{{chem}} \times A_{{total}}$  
...
if st.sidebar.checkbox("❓ Show Help & Troubleshooting", value=False):
    ...
    **Grain Size Method:**
    ...
    - $F_{{total}} = P_{{chem}} \times A_{{total}}$
    **Capillary Correction:**
    - $P_{{cap}} = 2\gamma/r$ with $r \approx d/4$ for growing FCC grains
    - $P_{{net}} = P_{{chem}} - P_{{cap}}$; positive $P_{{net}}$ drives LIQUID → FCC
    - Critical radius: $r^* = 2\gamma/P_{{chem}}$ (only when $P_{{chem}} > 0$)
    **Export Options:**
