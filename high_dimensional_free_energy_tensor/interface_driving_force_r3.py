...
def display_latex_theory():
    ...
    with st.expander("📋 View Theory in LaTeX Tabular Format", expanded=True):
        # NOTE: In f-strings, literal braces must be doubled {{ and }}
        latex_code = r"""
...
\end{table}
        ...
        with col_dl1:
            if st.button("📥 Copy LaTeX Code", key="btn_copy_latex"):
                st.code(latex_code, language="latex")
...
if g_liq is None or g_fcc is None:
    ...
    if area_mode == "Grain Size Derived (Sv x V)" and grain_size_um is not None:
        # NOTE: All curly braces in LaTeX inside f-strings are doubled {{ and }}
        st.markdown(f"""
...
with tab6:
    ...
    if g_liq is None or g_fcc is None:
        ...
        delta_G_v_norm = min(1.0, abs(delta_G_v) / 100)
