...
Optimized for Streamlit Cloud deployment – LAZY LOADING VERSION
...
FIXED: Cloud memory issues – only current temperature loaded
"""
...
# ================= LAZY LOADING: ONLY AVAILABLE TEMPERATURES (lightweight) =================
@st.cache_data
def get_available_temperatures():
    """Scan csv_files directory and return sorted list of temperatures (K)."""
    files = sorted(CSV_FILES_DIR.glob("Gibbs_*.csv"))
    temps = []
    for f in files:
        try:
...
temperatures = get_available_temperatures()
...
# ================= LAZY LOADING: Load and interpolate only for selected T =================
def load_data_for_T(T):
    """Read CSV for given temperature, validate, return DataFrame."""
    ...
    if len(df) == 0:
        st.warning(f"No valid compositions (sum=1) in {path}")
        return None
    return df
...
def build_interpolator(df, phase):
    """Build interpolator for a given phase ('LIQ' or 'FCC') from DataFrame."""
    ...
    if is_regular_grid(df):
        ...
        values_grid = df_sorted[f"G_{phase}"].values.reshape(len(co_vals), len(cr_vals), len(fe_vals))
        return RegularGridInterpolator(
            (co_vals, cr_vals, fe_vals), values_grid,
            bounds_error=False, fill_value=np.nan
        )
    else:
        return LinearNDInterpolator(points, values, fill_value=np.nan)

@st.cache_resource(ttl=3600, max_entries=5)  # Keep last 5 temperatures in cache
def get_interpolators_for_T(T):
    """Return (interp_liq, interp_fcc) for given temperature."""
    df = load_data_for_T(T)
    if df is None:
        return None, None
    interp_liq = build_interpolator(df, "LIQ")
    interp_fcc = build_interpolator(df, "FCC")
    # Free DataFrame memory
    del df
    return interp_liq, interp_fcc

def evaluate_point(x_co, x_cr, x_fe, T):
    """Evaluate G_LIQ and G_FCC at given composition and temperature."""
...
def display_latex_theory():
    ...
    with st.expander("📋 View Theory (Rendered Equations)", expanded=True):
        ...
        st.markdown(r"""
        ### 🌊 Capillary Pressure Correction (Advanced)
        ...
        | **Curvature Radius** | $r \approx \frac{D}{4}$ (local tip radius during growth) | Spherical cap approximation |
        | **Capillary Pressure** | $P_{\text{capillary}} = \frac{2\gamma}{r} \quad [\text{Pa}]$ | Kaptay model |
        | **Stereological Force** | $dF_{\text{net}} = P_{\text{net}} \cdot S_v \cdot dV \quad [\text{N}]$ | Underwood, Smith-Guttman |
        | **Physical Interpretation** | Smaller grains have *higher* $S_v$ but also *higher* capillary resistance. The net force per unit volume can be **~4x higher** in refined structures. | High-entropy GB studies |
        """)
        ...
        - ✅ Interfacial energy $\gamma$ range: 0.4–0.9 N/m for CoCrFeNi liquid/FCC (incoherent)
        """)
        ...
        8. Kaptay, G. *Calphad* **38**, 2012 (alloy interfacial energy model).
        9. Smith, C.S., Guttman, L. *Trans. AIME* **197**, 1953 (stereological relations).
        10. Hensler, J.H. *EPJP* **136**, 2021 (grain boundary resistivity factor k=3.24).
        11. Zhang et al. *Comp. Mater. Sci.* **202**, 2022 (Fe-Cr MD interfacial energy).
        12. Li et al. *J. Alloys Compd.* **821**, 2020 (M7C3 carbide γ=3.31 J/m²).
        13. Zhang et al. *Commun. Mater.* **4**, 2023 (high-entropy grain boundaries).
        """)
...
Capillary Pressure & $P_{\text{cap}} = 2\gamma / r$ & Laplace, Kaptay \\
Net Pressure & $P_{\text{net}} = \Delta G_v - P_{\text{cap}}$ & Christian \\
Area Density & $S_v = k/d$ & Smith-Guttman \\
Differential Force & $dF_{\text{net}} = P_{\text{net}} \cdot S_v \cdot dV$ & Underwood \\
...
# Temperature slider from available temps
...
if area_mode == "Direct Input (A)":
    ...
    shape_choice = st.sidebar.selectbox(
        ...
        index=1,
        help="k=2: spheres | k=3: tetrakaidecahedrons (metals) | k=6: cubes\n\nLiterature ranges:\n• Equiaxed polycrystals: k=2.37–2.82\n• Tetrakaidecahedron: k=2.90–3.24\n• HEA nanocrystalline: k=2.80–3.50"
    )
...
if use_capillary:
    with st.sidebar.expander("📚 Literature Ranges for γ", expanded=False):
        ...
        st.caption("Sources: multiple literature")
    ...
    if enable_uncertainty:
        st.sidebar.info("🔄 Running Monte Carlo simulation...")
        ...
        n_mc_samples = st.sidebar.selectbox(
            "Monte Carlo samples",
            [100, 300, 500, 1000],
            index=1,  # Default 300 for cloud speed
...
# Add a manual cache clearing button (cloud-friendly)
st.sidebar.divider()
if st.sidebar.button("🗑️ Clear all caches (free memory)", help="Removes all cached data and resets interpolators"):
    st.cache_data.clear()
...
if g_liq is None or g_fcc is None:
    ...
    # Load one sample DataFrame to show ranges
    df_sample = load_data_for_T(T)
    ...
    if use_capillary and grain_size_m is not None:
        ...
        col_p1.metric("Driving Pressure ΔGᵥ", f"{delta_G_v_MPa:.3f} MPa", help="Raw volumetric driving force: ΔG/Vₘ")
        col_p2.metric("Capillary Pressure", f"{P_capillary_MPa:.3f} MPa", help=f"P_cap = 2γ/r = 2×{gamma:.2f}/{curvature_r:.2e}", delta="resists growth", delta_color="inverse")
        col_p3.metric("Net Pressure P_net", f"{P_net_MPa:.3f} MPa", help="P_net = ΔGᵥ − P_capillary", delta=f"−{P_capillary_MPa:.2f} MPa", delta_color="normal")
        ...
        if enable_uncertainty and grain_size_m is not None:
            with st.expander("📊 Monte Carlo Uncertainty Results", expanded=True):
                gamma_range = (gamma * (1 - gamma_uncertainty_pct/100), gamma * (1 + gamma_uncertainty_pct/100))
                ...
                col_mc1.metric("P_net (95% CI)", f"{mc_results['P_net_mean']/1e6:.3f} MPa", delta=f"±{mc_results['P_net_std']/1e6:.3f} MPa", help=f"95% CI: [{P_net_ci[0]/1e6:.3f}, {P_net_ci[1]/1e6:.3f}] MPa")
                dF_ci = mc_results["dF_net_95ci"]
                col_mc2.metric("dF_net (95% CI)", f"{mc_results['dF_net_mean']:.2e} N", delta=f"±{mc_results['dF_net_std']:.2e} N", help=f"95% CI: [{dF_ci[0]:.2e}, {dF_ci[1]:.2e}] N")
                ...
                col_mc3.metric("Relative uncertainty", f"P: {rel_unc_P:.1f}%, F: {rel_unc_F:.1f}%", help="Coefficient of variation from Monte Carlo")
                
                col_plot1, col_plot2 = st.columns(2)
                with col_plot1:
                    fig_pnet = go.Figure(data=[go.Histogram(x=mc_results["P_net_samples"]/1e6, nbinsx=30, name="P_net distribution", marker_color=phase_color, opacity=0.7)])
                    fig_pnet.add_vline(x=P_net_MPa, line_dash="dash", line_color="red", annotation_text="Nominal")
                    fig_pnet.add_vline(x=P_net_ci[0]/1e6, line_dash="dot", line_color="gray")
                    fig_pnet.add_vline(x=P_net_ci[1]/1e6, line_dash="dot", line_color="gray", annotation_text="95% CI")
                    fig_pnet.update_layout(title="Net Pressure Distribution", xaxis_title="P_net (MPa)", yaxis_title="Frequency", height=300, showlegend=False)
                    st.plotly_chart(fig_pnet, width='stretch')
                with col_plot2:
                    fig_df = go.Figure(data=[go.Histogram(x=mc_results["dF_samples"], nbinsx=30, name="dF_net distribution", marker_color="#9467bd", opacity=0.7)])
                    fig_df.add_vline(x=dF_net, line_dash="dash", line_color="red", annotation_text="Nominal")
                    fig_df.add_vline(x=dF_ci[0], line_dash="dot", line_color="gray")
                    fig_df.add_vline(x=dF_ci[1], line_dash="dot", line_color="gray", annotation_text="95% CI")
                    fig_df.update_layout(title="Differential Force Distribution", xaxis_title="dF_net (N)", yaxis_title="Frequency", height=300, showlegend=False)
        ...
        **Capillary Correction Analysis:**
        ...
        col_p1.metric("Driving Pressure ΔGᵥ", f"{delta_G_v_MPa:.3f} MPa", help="Volumetric driving force: ΔG/Vₘ")
        col_p2.metric("SI units", f"{delta_G_v:.2e} N/m²", help="Equivalent to Pascals (Pa)")
    ...
    if area_mode == "Grain Size Derived (Sv x V)" and grain_size_um is not None:
        ...
        **Grain Size Method:** $F_{{total}} = P_{{net}} \times A_{{total}} = P_{{net}} \times S_v \times V$
        ...
        col_f3.metric("Net Force $F_{total}$", f"{net_force:.3e} N", help="Total thermodynamic driving force on all grain boundaries")
        ...
        if use_capillary and dF_net is not None:
            ...
            col_d3.metric("Differential Force dF_net", f"{dF_net:.3e} N", help="Force on a single μm³ element")
            st.info(f"""
        ...
        **Physical Interpretation:**
        ...
        col_f3.metric("Net Force F = ΔGᵥ × A", f"{net_force:.3e} N", help="Maximum thermodynamic driving force")
        st.info(f"""
        **Interpretation**:
...
with tab1:
    ...
    if max_val < 0.01:
        ...
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=valid_x, y=g_liq_scan, name="G_LIQUID", line=dict(color="#ff7f0e", width=2), mode='lines', fill='tozeroy', fillcolor='rgba(255,127,14,0.1)'))
        fig.add_trace(go.Scatter(x=valid_x, y=g_fcc_scan, name="G_FCC", line=dict(color="#1f77b4", width=2), mode='lines', fill='tozeroy', fillcolor='rgba(31,119,180,0.1)'))
        if scan_var == "x_Co":
            current_x = x_co
        elif scan_var == "x_Cr":
            current_x = x_cr
            ...
            current_x = x_fe
        fig.add_trace(go.Scatter(x=[current_x], y=[g_liq if g_liq else np.nan], name="Current: LIQUID", mode='markers', marker=dict(symbol='circle', size=10, color='#ff7f0e', line=dict(width=2, color='white'))))
        fig.add_trace(go.Scatter(x=[current_x], y=[g_fcc if g_fcc else np.nan], name="Current: FCC", mode='markers', marker=dict(symbol='square', size=10, color='#1f77b4', line=dict(width=2, color='white'))))
        fig.update_layout(title=f"Gibbs Energy vs {scan_var} at T={T} K (others={fixed_val:.2f})", xaxis_title=scan_var, yaxis_title="G (J/mol)", height=450, hovermode='x unified', legend=dict(orientation='h', yanchor='bottom', y=1.02, xanchor='right', x=1))
...
with tab2:
    ...
    if not valid_T:
        ...
        fig2 = go.Figure()
        fig2.add_trace(go.Scatter(x=valid_T, y=delta_G_list, mode="lines+markers", name="ΔG (J/mol)", yaxis="y1", line=dict(color="#2ca02c", width=2), marker=dict(size=6)))
        fig2.add_trace(go.Scatter(x=valid_T, y=delta_Gv_list, mode="lines+markers", name="ΔGᵥ (MPa)", yaxis="y2", line=dict(dash="dot", color="#d62728", width=2), marker=dict(symbol='square', size=6)))
        fig2.add_hline(y=0, line_dash="dash", line_color="gray", annotation_text="ΔG=0 boundary")
        fig2.update_layout(title=f"Driving Force vs Temperature<br><sup>Co:{x_co:.2f} Cr:{x_cr:.2f} Fe:{x_fe:.2f} Ni:{x_ni:.2f}</sup>", xaxis_title="Temperature (K)", yaxis=dict(title="ΔG (J/mol)", titlefont=dict(color="#2ca02c"), tickfont=dict(color="#2ca02c")), yaxis2=dict(title="ΔGᵥ (MPa)", titlefont=dict(color="#d62728"), tickfont=dict(color="#d62728"), overlaying="y", side="right"), height=500, hovermode='x unified', legend=dict(x=0.01, y=0.99, bgcolor='rgba(255,255,255,0.8)'))
    ...
    if max_val2 < 0.01:
        ...
        for xv in x_vals2:
            ...
            if gl is not None and gf is not None:
                ...
                dGv_vals.append(np.nan)
        fig3 = go.Figure()
        fig3.add_trace(go.Scatter(x=valid_x2, y=dGv_vals, mode="lines", fill="tozeroy", line=dict(color="#9467bd", width=2), fillcolor='rgba(148,103,189,0.2)', name="ΔGᵥ"))
        ...
        if current_dGv is not None:
            fig3.add_trace(go.Scatter(x=[current_x2], y=[current_dGv], mode='markers', name="Current point", marker=dict(symbol='star', size=12, color='#d62728', line=dict(width=2, color='white'))))
        fig3.update_layout(title=f"Driving Pressure ΔGᵥ vs {scan_var2} at T={T} K", xaxis_title=scan_var2, yaxis_title="ΔGᵥ (MPa)", height=450, hovermode='x unified')
    ...
    if df_display is not None:
        if filter_col != "None" and filter_op is not None and filter_val is not None:
            if filter_op == "==":
                ...
                df_display = df_display[df_display[filter_col] < filter_val]
        st.dataframe(df_display.style.format({"Co": "{:.3f}", "Cr": "{:.3f}", "Fe": "{:.3f}", "Ni": "{:.3f}", "G_LIQ": "{:.1f}", "G_FCC": "{:.1f}"}), width='stretch', height=500)
        col_exp1, col_exp2 = st.columns(2)
        with col_exp1:
            csv = df_display.to_csv(index=False)
            st.download_button(label="📥 Download Filtered Data as CSV", data=csv, file_name=f"CoCrFeNi_data_T{T}K.csv", mime="text/csv")
        with col_exp2:
...
with tab5:
    ...
    if g_liq is None or g_fcc is None:
        ...
        ids.append("root"); parents.append(""); labels.append("CoCrFeNi System"); values.append(1); colors.append("lightgray"); customdata.append(["root", 0, 0])
        ids.append(f"T_{T}"); parents.append("root"); labels.append(f"Temperature: {T} K"); values.append(1); colors.append("lightblue"); customdata.append([f"T_{T}", T, 0])
        elements = [("Co", x_co), ("Cr", x_cr), ("Fe", x_fe), ("Ni", x_ni)]
        for elem, frac in elements:
            elem_id = f"T_{T}_{elem}"
            ids.append(elem_id); parents.append(f"T_{T}"); labels.append(f"{elem}<br>{frac:.3f}"); values.append(frac); colors.append(element_colors[elem]); customdata.append([elem_id, elem, frac])
            force_id = f"{elem_id}_force"
            ids.append(force_id); parents.append(elem_id); display_force = net_force if use_capillary else delta_G_v * interface_area
            labels.append(f"Force: {display_force:.2e} N"); values.append(abs(display_force)); colors.append("gold"); customdata.append([force_id, display_force, display_force])
        fig_sb_current = go.Figure(go.Sunburst(ids=ids, parents=parents, labels=labels, values=values, marker=dict(colors=colors, line=dict(width=1, color='white')), branchvalues='total', hovertemplate='<b>%{label}</b><br>Value: %{value:.3e}<br><extra></extra>', customdata=customdata, insidetextorientation='radial', maxdepth=3))
        fig_sb_current.update_layout(title=dict(text=f'🌞 Current State: {T} K | Force = {display_force:.2e} N<br><sup>Outer: Temperature | Middle: Element (mole fraction) | Inner: Force (N)</sup>', font=dict(size=14), x=0.5), margin=dict(t=80, l=0, r=0, b=0), width=700, height=700, uniformtext=dict(minsize=10, mode='hide'))
        for elem, color in element_colors.items():
            fig_sb_current.add_annotation(x=1.05, y=1 - 0.05 * list(element_colors.keys()).index(elem), xref='paper', yref='paper', showarrow=False, text=f"• {elem}", font=dict(color=color, size=12), align='left')
        st.plotly_chart(fig_sb_current, width='stretch')
        if st.button("📥 Download Current Sunburst as PNG", key="btn_dl_current_sunburst"):
            try:
                ...
                st.download_button(label="Click to Download PNG", data=img_bytes, file_name=f"sunburst_current_T{T}K.png", mime="image/png")
            except Exception as e:
...
with tab6:
    ...
    if g_liq is None or g_fcc is None:
        ...
        phase_pref, phase_color, phase_emoji = get_phase_preference(delta_G)
        fig_radar = go.Figure(data=go.Scatterpolar(r=values, theta=categories, fill='toself', name=f'Current State {phase_emoji}', line=dict(color=phase_color, width=3), fillcolor=f'rgba({int(phase_color[1:3],16)}, {int(phase_color[3:5],16)}, {int(phase_color[5:7],16)}, 0.25)', hovertemplate='<b>Thermodynamic State</b><br>Composition: Co:%{r[0]:.3f} Cr:%{r[1]:.3f} Fe:%{r[2]:.3f} Ni:%{r[3]:.3f}<br>Temperature: %{r[4]:.3f} (norm) ≈ ' + f'{T} K<br>|Driving Pressure|: %{r[5]:.3f} (norm) ≈ {abs(delta_G_v_MPa):.2f} MPa<br>Interface Force: {abs(net_force):.2e} N<extra></extra>'))
        baseline_vals = [0.25, 0.25, 0.25, 0.25, 0.5, 0.3]
        if use_capillary and P_capillary is not None:
            baseline_vals = [0.25, 0.25, 0.25, 0.25, 0.5, 0.3, 0.1, 0.3]
        fig_radar.add_trace(go.Scatterpolar(r=baseline_vals, theta=categories, fill='none', name='Equiatomic Reference', line=dict(color='gray', width=1.5, dash='dot'), opacity=0.6))
        ...
        fig_radar.update_layout(title=dict(text=radar_title, font=dict(size=14), x=0.5), polar=dict(radialaxis=dict(visible=True, range=[0, 1.1], tickfont=dict(size=9), gridcolor='lightgray', linecolor='gray'), angularaxis=dict(tickfont=dict(size=10, color='darkgray'), rotation=90, direction='clockwise', gridcolor='lightgray'), bgcolor='rgba(240,240,240,0.3)'), showlegend=True, legend=dict(orientation='h', yanchor='bottom', y=-0.15, xanchor='center', x=0.5), margin=dict(t=70, l=30, r=30, b=50), width=550, height=550, annotations=[dict(text=f"ΔGᵥ = {delta_G_v_MPa:.1f} MPa<br>F = {net_force:.2e} N", x=0.5, y=-0.22, xref='paper', yref='paper', showarrow=False, font=dict(size=10, color=phase_color), bgcolor=f'rgba({int(phase_color[1:3],16)}, {int(phase_color[3:5],16)}, {int(phase_color[5:7],16)}, 0.1)', borderpad=4, bordercolor=phase_color)])
            ...
            **Current Metrics:**
            ...
            if st.button("📥 Download Radar as PNG", key="btn_dl_radar"):
                try:
                    ...
                    st.download_button(label="Click to Download", data=img_bytes, file_name=f"radar_CoCrFeNi_T{T}K.png", mime="image/png")
                except:
...
with tab7:
    ...
    if g_liq is None or g_fcc is None:
        ...
        for gs_m in grain_sizes_m:
            ...
            Svs.append(Sv_val)
        fig_gs1 = go.Figure()
        fig_gs1.add_trace(go.Scatter(x=grain_sizes_um, y=P_caps, mode="lines", name="P_capillary", line=dict(color="#d62728", width=2, dash="dash")))
        fig_gs1.add_trace(go.Scatter(x=grain_sizes_um, y=P_nets, mode="lines", name="P_net", line=dict(color="#2ca02c", width=2)))
        fig_gs1.add_hline(y=delta_G_v_MPa, line_dash="dot", line_color="blue", annotation_text=f"ΔGᵥ = {delta_G_v_MPa:.1f} MPa")
        fig_gs1.update_layout(title="Net Pressure vs Grain Size", xaxis_title="Grain Size D (μm)", yaxis_title="Pressure (MPa)", height=400, hovermode="x unified")
        st.plotly_chart(fig_gs1, width='stretch')
        fig_gs2 = go.Figure()
        fig_gs2.add_trace(go.Scatter(x=grain_sizes_um, y=dF_nets, mode="lines", fill="tozeroy", line=dict(color="#9467bd", width=2), fillcolor="rgba(148,103,189,0.2)", name="dF_net"))
        fig_gs2.update_layout(title=f"Differential Force on {dV_um3 if dV_um3 else 1:.1f} μm³ Element vs Grain Size", xaxis_title="Grain Size D (μm)", yaxis_title="dF_net (N)", height=400, hovermode="x unified")
...
with tab8:
    ...
    if g_liq is None or g_fcc is None:
        ...
        with col_s1:
            ...
            fig_gamma = go.Figure()
            fig_gamma.add_trace(go.Scatter(x=gamma_sweep, y=np.array(P_net_sweep_gamma)/1e6, mode="lines", fill="tozeroy", line=dict(color="#d62728", width=2), fillcolor="rgba(214,39,40,0.2)", name="P_net(γ)"))
            fig_gamma.add_vline(x=gamma, line_dash="dash", line_color="black", annotation_text=f"Current: {gamma:.2f}")
            fig_gamma.update_layout(title="Net Pressure vs Interfacial Energy γ", xaxis_title="γ (N/m)", yaxis_title="P_net (MPa)", height=350)
        ...
        with col_s2:
            ...
            fig_k = go.Figure()
            fig_k.add_trace(go.Scatter(x=k_sweep, y=dF_sweep_k, mode="lines", fill="tozeroy", line=dict(color="#9467bd", width=2), fillcolor="rgba(148,103,189,0.2)", name="dF_net(k)"))
            fig_k.add_vline(x=shape_factor, line_dash="dash", line_color="black", annotation_text=f"Current: {shape_factor:.2f}")
            fig_k.update_layout(title="Differential Force vs Shape Factor k", xaxis_title="k (dimensionless)", yaxis_title="dF_net (N)", height=350)
        ...
        if grain_size_m is not None and use_capillary:
            ...
            col_sum1.metric("P_net nominal", f"{P_nom:.2f} MPa", delta=f"MC mean: {P_mean:.2f} ± {P_std:.2f} MPa")
            F_nom = dF_net
            F_mean = mc_quick["dF_net_mean"]
            F_std = mc_quick["dF_net_std"]
            col_sum2.metric("dF_net nominal", f"{F_nom:.2e} N", delta=f"MC mean: {F_mean:.2e} ± {F_std:.2e} N")
            dP_dgamma = (compute_capillary_pressure(gamma*1.01, curvature_r) - compute_capillary_pressure(gamma*0.99, curvature_r)) / (0.02*gamma) / 1e6
            col_sum3.metric("∂P_net/∂γ", f"{-dP_dgamma:.3f} MPa/(N/m)", help="Sensitivity of net pressure to interfacial energy")
            cap_fraction = abs(P_capillary_MPa / delta_G_v_MPa) * 100 if delta_G_v_MPa != 0 else 0
            col_sum4.metric("Capillary contribution", f"{cap_fraction:.1f}%", help="Fraction of driving pressure offset by capillarity")
            st.markdown(f"""
            **Interpretation:**
            ...
            - For **reliable predictions**, constrain γ to literature ranges: **0.4–0.9 N/m** for CoCrFeNi
            """)
        else:
...
3. Zhang et al. *Comp. Mater. Sci.* **202**, 2022 (Fe-Cr MD interfacial energy)
...
8. Kaptay, G. *Calphad* **38**, 2012 (alloy interfacial energy model)
9. Smith, C.S. & Guttman, L. *Trans. AIME* **197**, 1953 (stereological relations)
10. Hensler, J.H. *EPJP* **136**, 2021 (grain boundary factor k=3.24)
11. Underwood, E.E. *Quantitative Stereology*, Addison-Wesley (grain shape factors & Sv)
12. Zhang et al. *Commun. Mater.* **4**, 2023 (high-entropy grain boundaries)
