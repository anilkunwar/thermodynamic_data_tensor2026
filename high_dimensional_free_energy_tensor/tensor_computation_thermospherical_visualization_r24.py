...
def cpd_als_4d(tensor, rank, max_iter=100, tol=1e-6, use_weighted=False, reg=1e-8):
    ...
    if L == 31:  # Standard 31-temperature grid
        ...
        if rank > 4:
            D[:, 4:] = np.random.randn(L, rank-4) * 0.01  # Higher orders: random
    else:
    ...
    if st.button("⚙️ Run CPD-ALS (may take 1-2 min for large tensors)", use_container_width=True):
        ...
        for phase_name, phase_key, tensor_phase in phases_to_run:
            with st.spinner(f"Running CP-ALS for {phase_name} with R={R_test}..."):
                ...
                A, B, C, D, lam, recon_norm, meta = cpd_als_4d(tensor_norm, R_test, max_iter=max_iter, tol=1e-5, use_weighted=True, reg=1e-8)
        ...
        with col1:
            if st.button("🚀 Run CPD (Rank=6, both phases)", use_container_width=True, key="run_cpd_factors"):
                ...
                with st.spinner("Running CPD for LIQUID..."):
                    ...
                    A_liq, B_liq, C_liq, D_liq, lam_liq, recon_norm_liq, meta_liq = cpd_als_4d(tensor_norm, R_test, max_iter)
                ...
                with st.spinner("Running CPD for FCC..."):
                    ...
                    A_fcc, B_fcc, C_fcc, D_fcc, lam_fcc, recon_norm_fcc, meta_fcc = cpd_als_4d(tensor_norm, R_test, max_iter)
        ...
        if 'quadratic_coeffs_fcc' in st.session_state:
            ...
            if st.button("Generate Full Comparison Analysis", type="primary", key="quad_full_comp"):
                with st.spinner("Generating comprehensive comparison..."):
                    ...
                    st.plotly_chart(fig_comp, width='stretch', key="plotly_quad_comp")
                    ...
                    st.plotly_chart(fig_3d, width='stretch', key="plotly_quad_3d")

                    # Error metrics dashboard
                    st.subheader("📈 Error Analysis Dashboard")
                    fig_err = plot_error_metrics_dashboard(verify_df_fcc)
                    st.plotly_chart(fig_err, width='stretch', key="plotly_quad_err")
            ...
            if SCIPY_AVAILABLE:
                ...
                st.plotly_chart(fig_quad_morph, width='stretch', key="plotly_quad_morph")
