...
with tab_cpd:
    ...
    if st.button("Run CPD-ALS Decomposition", use_container_width=True, type="primary"):
        with st.spinner(f"Running CP-ALS for {phase_for_tensor} with R={R_test}..."):
            ...
            _phase_key_cache = "LIQ" if phase_for_tensor == "G_LIQUID" else "FCC"
            st.session_state[f'recon_norm_{_phase_key_cache}'] = recon
            # -----------------------------------------------------------------------
...
with tab_recon:
    ...
    if not st.session_state.get(f'cpd_completed_{phase_key}', False):
        ...
        _cache_key = f'recon_norm_{phase_key.lower()}'
        if _cache_key in st.session_state:
            recon_norm = st.session_state[_cache_key]
        else:
            # Fallback: recompute from factors (slow but correct)
