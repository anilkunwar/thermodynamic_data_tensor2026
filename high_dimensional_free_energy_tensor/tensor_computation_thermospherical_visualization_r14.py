...
def plot_d_factor_matrix(D_liq, D_fcc, T_vals, lam_liq, lam_fcc, R=6, mode='heatmap'):
    """
    Visualize temperature factor matrices D_liq and D_fcc.

    Args:
        D_liq, D_fcc: Temperature factor matrices (n_T × R)
        T_vals: Temperature grid values (from filenames, e.g. [700, 800, ..., 3300])
        lam_liq, lam_fcc: Component weights
        R: Number of components
        mode: 'heatmap' for full matrix, 'single_T' for bar chart at one T
    ...
    n_T = len(T_vals)
    T_labels = [f"{int(t)}K" for t in T_vals]

    fig = make_subplots(
        rows=1, cols=2,
        subplot_titles=('LIQUID Phase: D[T,r] × λᵣ', 'FCC Phase: D[T,r] × λᵣ'),
        horizontal_spacing=0.12
    )

    # Weighted D matrices
    D_liq_weighted = D_liq * lam_liq[None, :R] if R <= len(lam_liq) else D_liq * lam_liq[None, :D_liq.shape[1]]
    D_fcc_weighted = D_fcc * lam_fcc[None, :R] if R <= len(lam_fcc) else D_fcc * lam_fcc[None, :D_fcc.shape[1]]

    # Common color scale
    zmin = min(np.min(D_liq_weighted), np.min(D_fcc_weighted))
    zmax = max(np.max(D_liq_weighted), np.max(D_fcc_weighted))

    r_labels = [f"r={r+1}" for r in range(min(R, D_liq.shape[1]))]

    # LIQUID heatmap
    fig.add_trace(go.Heatmap(
        z=D_liq_weighted[:, :min(R, D_liq.shape[1])],
        x=r_labels,
        y=T_labels,
        colorscale='RdBu_r',
        zmid=0,
        zmin=zmin, zmax=zmax,
        colorbar=dict(title="λ·D(T)", thickness=15, len=0.8, x=0.45),
        hovertemplate="T=%{y}<br>Component=%{x}<br>λ·D=%{z:.4f}<extra></extra>"
    ), row=1, col=1)

    # FCC heatmap
    fig.add_trace(go.Heatmap(
        z=D_fcc_weighted[:, :min(R, D_fcc.shape[1])],
        x=r_labels,
        y=T_labels,
        colorscale='RdBu_r',
        zmid=0,
        zmin=zmin, zmax=zmax,
        colorbar=dict(title="λ·D(T)", thickness=15, len=0.8, x=1.0),
        hovertemplate="T=%{y}<br>Component=%{x}<br>λ·D=%{z:.4f}<extra></extra>"
    ), row=1, col=2)

    fig.update_layout(
        height=max(400, min(800, n_T * 20)),
        title_text="Temperature Factor Matrices D(T,r) — Extracted from Gibbs_*K.csv filenames",
        showlegend=False
    )
...
def plot_d_at_single_T(D_liq, D_fcc, T_vals, lam_liq, lam_fcc, T_select, R=6):
    """
    Bar chart of D factors at a single temperature, both phases side by side.

    Args:
        T_select: Specific temperature value (must be in T_vals)
    """
    T_idx = np.argmin(np.abs(T_vals - T_select))
    T_actual = T_vals[T_idx]

    r_labels = [f"r={r+1}" for r in range(min(R, D_liq.shape[1], D_fcc.shape[1]))]
    R_eff = len(r_labels)
    ...
    for r in range(R_eff):
        val_liq = lam_liq[r] * D_liq[T_idx, r] if r < len(lam_liq) and r < D_liq.shape[1] else 0
        fig.add_trace(go.Bar(
            x=[r_labels[r]],
            y=[val_liq],
            name=f"LIQUID r={r+1}",
            marker_color=colors[r % len(colors)],
            opacity=0.85,
            width=0.35,
            offset=-0.2,
            showlegend=(r == 0),
            legendgroup="liq",
            hovertemplate=f"LIQUID r={r+1}<br>T={T_actual}K<br>λ·D={val_liq:.4f}<extra></extra>"
        ))

    # FCC bars
    for r in range(R_eff):
        val_fcc = lam_fcc[r] * D_fcc[T_idx, r] if r < len(lam_fcc) and r < D_fcc.shape[1] else 0
        fig.add_trace(go.Bar(
            x=[r_labels[r]],
            y=[val_fcc],
            name=f"FCC r={r+1}",
            ...
            showlegend=(r == 0),
            legendgroup="fcc",
            hovertemplate=f"FCC r={r+1}<br>T={T_actual}K<br>λ·D={val_fcc:.4f}<extra></extra>"
        ))

    fig.update_layout(
        title=f"Temperature Factors at T = {T_actual} K (from Gibbs_{int(T_actual)}K.csv)",
        xaxis_title="CPD Component",
        yaxis_title="Weighted Factor λ·D(T,r)",
        barmode='group',
        height=500,
        legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="center", x=0.5)
    )

    # Add AM-relevant temperature annotations as vertical reference lines
    am_temps = {
        'Fe Curie T': 1043,
        'Solidus': 1600,
        'Melt Pool': 2800,
    ...
    for label, T_val in am_temps.items():
        if abs(T_actual - T_val) < 50:
            fig.add_hline(y=0, line_dash="dot", line_color="gold", opacity=0.3,
...
def render_factor_matrix_visualisation(A_liq, B_liq, C_liq, D_liq, lam_liq,
    ...
    CRITICAL FIX: All grid value arrays (co_vals, cr_vals, fe_vals, T_vals) are 
    converted to numpy arrays to prevent TypeError when doing arithmetic 
    (e.g., fe_vals - fixed_fe requires array, not list).

    Temperature information comes from Gibbs_*K.csv filenames and flows into:
    - T_vals: list of temperatures extracted from filenames
    - D_liq, D_fcc: temperature factor matrices (n_T × R)
    """
    # === CONVERT TO NUMPY ARRAYS (prevents TypeError: list - float) ===
    ...
    st.markdown(r"""
    The CPD factorises Gibbs energy into separable components:
    $$G \approx \sum_{r=1}^{R} \lambda_r \, A_r(x_{Co}) \, B_r(x_{Cr}) \, C_r(x_{Fe}) \, D_r(T)$$

    **Temperature information** comes from filenames (`Gibbs_700K.csv` → `T=700K`) 
    and is encoded in the **D matrix** — one column per CPD component, one row per temperature.
    """)

    phase_choice = st.radio("Select phase for composition factors", ["LIQUID", "FCC"], 
    ...
    # ─────────────────────────────────────────
    # SECTION 1: Composition Factor Profiles (A, B, C)
    # ─────────────────────────────────────────
    st.subheader("📊 1. Composition Factor Profiles (A, B, C)")
    st.caption("Weighted by λᵣ. Each column = one CPD component r. Each row = one element.")
    fig_profiles = plot_factor_profiles(A, B, C, lam, co_vals, cr_vals, fe_vals, R)
    st.plotly_chart(fig_profiles, use_container_width=True)

    # ─────────────────────────────────────────
    # SECTION 2: Temperature Factor Matrix (D) — NEW
    # ─────────────────────────────────────────
    st.subheader("🌡️ 2. Temperature Factor Matrix D(T,r)")
    st.caption("Each row = one temperature (from Gibbs_*K.csv filenames). Each column = one CPD component.")

    d_view_mode = st.radio("View mode", ["Full Matrix (All T)", "Single Temperature"], 
                          index=0, horizontal=True, key="d_view_mode")

    if d_view_mode == "Full Matrix (All T)":
        fig_d_matrix = plot_d_factor_matrix(D_liq, D_fcc, T_vals, lam_liq, lam_fcc, R, mode='heatmap')
        st.plotly_chart(fig_d_matrix, use_container_width=True)
        st.info("""
        **Reading the D matrix:**
        - **r=1** (leftmost column): ~constant across T → baseline enthalpy offset
        - **r=2**: linear trend with T → entropy term (−S·T), strongest at high T
        - **r=3**: curvature/kink → heat capacity + magnetic transitions (Fe Curie ~1043K)
        - **r=4–6**: fine structure → binary/ternary interaction temperature dependence
        """)
    else:
        ...
        with col_t1:
            T_select = st.selectbox(
                "Select Temperature", 
                [f"{int(t)} K" for t in T_vals],
                index=len(T_vals)//2
            )
            T_select_val = float(T_select.replace(" K", ""))
        with col_t2:
            fig_d_single = plot_d_at_single_T(D_liq, D_fcc, T_vals, lam_liq, lam_fcc, T_select_val, R)
            st.plotly_chart(fig_d_single, use_container_width=True)

        # Show which filename this corresponds to
        st.caption(f"📁 Data source: `Gibbs_{int(T_select_val)}K.csv`")

    # ─────────────────────────────────────────
    # SECTION 3: Temperature Factors with AM Thermal Cycle (existing)
    # ─────────────────────────────────────────
    st.subheader("🔥 3. Temperature Factors + AM Thermal Cycle Overlay")
    st.caption("Weighted D[T,r] curves with AM process temperature annotations.")
    fig_temp = plot_temperature_factors_am(D_liq, D_fcc, T_vals, lam_liq, lam_fcc, R)
    st.plotly_chart(fig_temp, use_container_width=True)

    # ─────────────────────────────────────────
    # SECTION 4: Single-Component Heatmap (existing, fixed)
    # ─────────────────────────────────────────
    st.subheader("🗺️ 4. Single-Component Spatial Heatmap (2D Slice)")
    st.caption("Visualise one CPD component's contribution in the Co-Cr plane at fixed Fe and T.")
    col1, col2, col3 = st.columns(3)
    with col1:
        r_select = st.selectbox("Component r", list(range(1, R+1)), index=min(2, R-1), key="comp_r")
    with col2:
        fixed_Fe = st.slider("Fixed Fe mole fraction", float(fe_vals.min()), float(fe_vals.max()), 
                            float(np.median(fe_vals)), float(fe_vals[1]-fe_vals[0]) if len(fe_vals)>1 else 0.01,
                            key="fixed_fe")
    with col3:
        fixed_T = st.slider("Fixed Temperature (K)", float(T_vals.min()), float(T_vals.max()),
    ...
    # ─────────────────────────────────────────
    # SECTION 5: Reconstruction Quality Check (existing, fixed)
    # ─────────────────────────────────────────
    st.subheader("✅ 5. Reconstruction Quality Check")
    st.caption("Compare CPD reconstruction against original CALPHAD data.")
    if st.button("Evaluate reconstruction error on a 2D slice (LIQUID only)", key="eval_recon"):
...
# =============================================
tab_main, tab_tensor, tab_factors, tab_am = st.tabs([
    "🎨 Phase Visualization", 
...
with tab_factors:
    ...
    if not factors_available:
        ...
        col1, col2 = st.columns(2)
        with col1:
            if st.button("🚀 Run CPD (Rank=6, both phases)", use_container_width=True, key="run_cpd_factors"):
                tdt_data = build_tensor_data(df)
                R_test = 6
                max_iter = 100
                ...
                st.success("✅ CPD completed! Factors saved. Refreshing...")
                st.rerun()
        with col2:
            if st.button("🎲 Use demo factors (quick preview)", use_container_width=True, key="demo_factors"):
                tdt_data = build_tensor_data(df)
                n_co, n_cr, n_fe, n_T = tdt_data['dims']
                R_demo = 6
                np.random.seed(42)
