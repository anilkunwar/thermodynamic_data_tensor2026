...
def compute_quadratic_coefficients_from_cpd(
    ...
    V_m=6.93e-6  # molar volume in m^3/mol
):
    """
    Compute the quadratic expansion coefficients from CPD factor matrices.

    The quadratic form is: G/V_m ≈ A_Co*(c_Co - c_eq_Co)^2 + A_Cr*(c_Cr - c_eq_Cr)^2 
    ...
    Returns:
    --------
    dict with quadratic coefficients A_Co, A_Cr, A_Fe, A_T
    """
    R = len(lam)
    ...
    # Compute A_Co: second derivative w.r.t. Co
    A_Co_sum = 0.0
    for r in range(R):
    ...
    # Compute A_Cr: second derivative w.r.t. Cr
    A_Cr_sum = 0.0
    for r in range(R):
    ...
    # Compute A_Fe: second derivative w.r.t. Fe
    A_Fe_sum = 0.0
    for r in range(R):
        A_val = A_funcs[r](c_eq_co)
        B_val = B_funcs[r](c_eq_cr)
    ...
    # Compute A_T: second derivative w.r.t. Temperature
    A_T_sum = 0.0
    for r in range(R):
        A_val = A_funcs[r](c_eq_co)
        B_val = B_funcs[r](c_eq_cr)
    ...
    # Compute G at equilibrium (constant term)
    G_eq = 0.0
    for r in range(R):
...
# =============================================
tab_main, tab_tensor, tab_factors, tab_quadratic, tab_am = st.tabs(["🎨 Phase Visualization", "📊 Tensor Decomposition (CPD)", "🔢 Factor Matrices", "📐 Quadratic Expansion (Phase-Field)", "🏭 AM Design Assistant"])
    ...
    if not factors_available:
        ...
        with col1:
            if st.button("🚀 Run CPD (Rank=6, both phases)", use_container_width=True, key="run_cpd_factors"):
                ...
                st.rerun()
        with col2:
            if st.button("🎲 Use demo factors", use_container_width=True, key="demo_factors"):
                tdt_data = build_tensor_data(df)
                n_co, n_cr, n_fe, n_T = tdt_data['dims']
...
with tab_quadratic:
    ...
    Instead of querying the full 4D CPD tensor, the bulk free-energy density is parameterized via a rigorous quadratic expansion around the equilibrium point $(\mathbf{c}_{eq}, T_m)$:

    $$ rac{G}{V_m} pprox rac{G_{eq}}{V_m} + A_{Co}(c_{Co} - c_{eq,Co})^2 + A_{Cr}(c_{Cr} - c_{eq,Cr})^2 + A_{Fe}(c_{Fe} - c_{eq,Fe})^2 + A_T(T - T_m)^2 $$

    This captures the essential curvature of the Gibbs energy landscape (driving force for phase transformation) while bypassing the computational overhead of full tensor interpolation.
    """)
    ...
    if 'A_liq' not in st.session_state or 'A_fcc' not in st.session_state:
        ...
        col1, col2 = st.columns(2)
        with col1:
            c_eq_co = st.number_input("Equilibrium $x_{Co}$", 0.0, 1.0, 0.25, 0.01, key="quad_c_eq_co")
            c_eq_cr = st.number_input("Equilibrium $x_{Cr}$", 0.0, 1.0, 0.25, 0.01, key="quad_c_eq_cr")
            c_eq_fe = st.number_input("Equilibrium $x_{Fe}$", 0.0, 1.0, 0.25, 0.01, key="quad_c_eq_fe")
        with col2:
            T_m = st.number_input("Equilibrium Temperature $T_m$ (K)", 700, 3300, 1500, 50, key="quad_T_m")
            V_m = st.number_input("Molar Volume $V_m$ (m³/mol)", 0.0, 1e-4, 6.93e-6, 1e-7, format="%.2e", key="quad_V_m")
        ...
        if st.button("🔬 Compute Quadratic Coefficients", use_container_width=True, type="primary", key="quad_compute"):
            with st.spinner("Computing Hessian from CPD factors..."):
                ...
                coeffs_fcc = compute_quadratic_coefficients_from_cpd(
                    ...
                    c_eq_co, c_eq_cr, c_eq_fe, T_m, V_m
                )
        ...
        if 'quadratic_coeffs_fcc' in st.session_state:
            ...
            with col1:
                ...
                st.latex(r"A_{Co} = %.3e 	ext{ J/m}^3" % coeffs_fcc['A_Co'])
                st.latex(r"A_{Cr} = %.3e 	ext{ J/m}^3" % coeffs_fcc['A_Cr'])
                st.latex(r"A_{Fe} = %.3e 	ext{ J/m}^3" % coeffs_fcc['A_Fe'])
                st.latex(r"A_{T} = %.3e 	ext{ J/(m}^3\cdot	ext{K}^2)" % coeffs_fcc['A_T'])
                st.latex(r"G_{eq} = %.3e 	ext{ J/mol}" % coeffs_fcc['G_eq'])
            with col2:
                st.markdown("**LIQUID Phase**")
                st.latex(r"A_{Co} = %.3e 	ext{ J/m}^3" % coeffs_liq['A_Co'])
                st.latex(r"A_{Cr} = %.3e 	ext{ J/m}^3" % coeffs_liq['A_Cr'])
                st.latex(r"A_{Fe} = %.3e 	ext{ J/m}^3" % coeffs_liq['A_Fe'])
                st.latex(r"A_{T} = %.3e 	ext{ J/(m}^3\cdot	ext{K}^2)" % coeffs_liq['A_T'])
            ...
            st.plotly_chart(fig_slice, use_container_width=True, key="plotly_quad_slice")
            ...
            if st.button("Generate Full Comparison Analysis", type="primary", key="quad_full_comp"):
                with st.spinner("Generating comprehensive comparison..."):
                    ...
                    fig_err = plot_error_metrics_dashboard(verify_df_fcc)
                    st.plotly_chart(fig_err, use_container_width=True, key="plotly_quad_err")
            ...
            if SCIPY_AVAILABLE:
                ...
                if coeffs_sh_fcc_full is not None:
                    ...
                    fig_quad_morph.add_trace(go.Surface(x=R_surf*np.sin(PH)*np.cos(TH), y=R_surf*np.sin(PH)*np.sin(TH), z=R_surf*np.cos(PH), surfacecolor=G_sh, colorscale="Blues", opacity=0.3, name="Full CPD (FCC)", showscale=False))
                if coeffs_sh_liq_full is not None:
                    ...
                    fig_quad_morph.add_trace(go.Surface(x=R_surf*np.sin(PH)*np.cos(TH), y=R_surf*np.sin(PH)*np.sin(TH), z=R_surf*np.cos(PH), surfacecolor=G_sh, colorscale="Reds", opacity=0.3, name="Full CPD (LIQUID)", showscale=False))
                ...
                if coeffs_sh_fcc_quad is not None:
                    ...
                    R_surf = get_fcc_radius(G_sh, sh_R_fixed_quad, T_factor_morph)
                    fig_quad_morph.add_trace(go.Surface(x=R_surf*np.sin(PH)*np.cos(TH), y=R_surf*np.sin(PH)*np.sin(TH), z=R_surf*np.cos(PH), surfacecolor=G_sh, colorscale="Blues", opacity=0.8, name="Quadratic (FCC)", contours=dict(z=dict(show=True, color="darkblue", width=2))))
                if coeffs_sh_liq_quad is not None:
                    G_sh = reconstruct_sh_surface(TH, PH, coeffs_sh_liq_quad, 3)
                    R_surf = get_liquid_radius(G_sh, sh_R_fixed_quad, T_factor_morph)
                    fig_quad_morph.add_trace(go.Surface(x=R_surf*np.sin(PH)*np.cos(TH), y=R_surf*np.sin(PH)*np.sin(TH), z=R_surf*np.cos(PH), surfacecolor=G_sh, colorscale="Reds", opacity=0.8, name="Quadratic (LIQUID)", contours=dict(z=dict(show=True, color="darkred", width=2))))

                fig_quad_morph.update_layout(
                    title=f"Temperature-Morphed Gibbs Landscape at T = {T_morph} K",
                    scene=dict(xaxis=dict(title="x_Co", range=[0, 1]), yaxis=dict(title="x_Cr", range=[0, 1]), zaxis=dict(title="x_Fe", range=[0, 1]), aspectmode="cube"),
                    height=700
                )
                st.plotly_chart(fig_quad_morph, use_container_width=True, key="plotly_quad_morph")
            ...
            **Taylor Expansion Interpretation:**
            ...
            $$G_{	ext{quad}}(\mathbf{c}, T) = G(\mathbf{c}_{eq}, T_m) + rac{1}{2}\sum_{lpha} \left.rac{\partial^2 G}{\partial c_lpha^2}
ight|_{eq} (c_lpha - c_{lpha,eq})^2 + rac{1}{2}\left.rac{\partial^2 G}{\partial T^2}
ight|_{T_m} (T - T_m)^2$$
